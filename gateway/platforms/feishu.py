...
class FeishuAdapter(BasePlatformAdapter):
    ...
    async def send_exec_approval(
        ...
        try:
            ...
            if result.success:
                ...
                # Mobile users cannot use card button callbacks on Feishu mobile.
                # Send a text fallback so they can approve via text reply.
                cmd_preview_text = command[:200] + "..." if len(command) > 200 else command
                text_msg = (
                    f"⚠️ **危险命令需要你的审批：**\n"
                    f"```\n{cmd_preview_text}\n```\n"
                    f"原因：{description}\n\n"
                    f"📱 **手机端用户：请在输入框输入数字回复审批：**\n"
                    f"**1** 或 **/approve** — 仅执行这一次\n"
                    f"**2** 或 **/approve session** — 本次会话允许\n"
                    f"**3** 或 **/approve always** — 永久允许\n"
                    f"**0** 或 **/deny** — 拒绝执行\n\n"
                    f"💻 **桌面端用户：也可直接点击上方卡片按钮审批**"
                )
                try:
                    await self._feishu_send_with_retry(
                        chat_id=chat_id,
                        msg_type="text",
                        payload=json.dumps({"text": text_msg}, ensure_ascii=False),
                        reply_to=None,
                        metadata=metadata,
                    )
                except Exception as _e:
                    logger.warning(
                        "[Feishu] Failed to send approval text fallback: %s", _e
                    )
            return result
        except Exception as exc:
    ...
    def _on_card_action_trigger(self, data: Any) -> Any:
        ...
        All paths wrapped in try/except to ensure the SDK always receives
        a valid P2CardActionTriggerResponse — never crashes mid-dispatch.
        """
        try:
            return self._on_card_action_trigger_safe(data)
        except Exception:
            logger.exception("[Feishu] Card action trigger crashed — SDK will use empty response")
            return P2CardActionTriggerResponse() if P2CardActionTriggerResponse else None

    def _on_card_action_trigger_safe(self, data: Any) -> Any:
        """Core card action dispatch (safe — callers wrap in try/except)."""
        loop = self._loop
        if not self._loop_accepts_callbacks(loop):
        ...
        if hermes_action:
            return self._handle_approval_card_action(event=event, action_value=action_value, loop=loop, data=data)

        # Route non-approval card actions to async handler
